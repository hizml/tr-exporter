...
按 Tracker 批量导出 [Transmission](https://transmissionbt.com/) 的 `.torrent` 种子文件的交互式脚本。

特别适合 **QNAP / 群晖等 NAS 上的 Docker 版 Transmission**——无需 SSH，只需在 Container Station / 容器管理器的「终端」里运行即可。

## 解决什么问题

Transmission 自带界面没有「按 Tracker 一键导出」功能。当你需要：
...
- 🖥 **纯交互式**：引导输入 RPC 地址、账号密码、选择 Tracker，不用改脚本
- 🎯 **按 Tracker 筛选**：列出现有 Tracker，按序号选择 / 关键字匹配 / 全部导出
- 🔍 **智能定位种子目录**：自动跳过 `/kettu/templates/torrents` 等「假目录」，只认含 40 位 hex 种子文件的真实目录
...
```sh
cat > /tmp/tr-export.sh <<'EOF'
# ……（此处粘贴 tr-export.sh 的完整内容）……
EOF
sh /tmp/tr-export.sh
```

> 💡 也可以把 `tr-export.sh` 放进容器已映射的共享文件夹（如 `/config`），用文件管理器从 NAS 上直接编辑/运行，省去 heredoc。
...
【RPC 地址】
  常见预设：
   1) http://127.0.0.1:9091/transmission/rpc   (容器内本机，最常用)
   2) http://localhost:9091/transmission/rpc
   3) http://NAS_IP:9091/transmission/rpc       (从容器外访问 NAS)
   4) 自定义输入
选择序号或直接输入地址 (留空=默认 1):        ← 回车即用默认 127.0.0.1:9091
